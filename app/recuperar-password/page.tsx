...
export default function RecuperarPasswordPage() {
  ...
  if (!valid) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 text-center">
        <p className="text-lg font-bold text-[#14141A] mb-2">Enlace inválido o expirado</p>
        <p className="text-sm text-slate-500 mb-6">Solicita un nuevo enlace de recuperación desde la app.</p>
        <Link href="/" className="text-sm font-semibold text-[#14141A] hover:underline">Volver al inicio</Link>
      </div>
    )
  }
  ...
  if (done) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 text-center">
        <p className="text-lg font-bold text-[#00A846] mb-2">✓ Contraseña actualizada</p>
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      <div className="bg-[#14141A] px-6 pt-12 pb-6">
        ...
        <p className="text-white/50 text-sm mt-1">Crea una contraseña segura para tu cuenta</p>
      </div>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <button
              type="button"
              onClick={() => setShow(s => !s)}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-semibold text-[#94A3B8] hover:text-slate-700"
            >
              {show ? 'Ocultar' : 'Ver'}
            </button>
          </div>
      ...
      <div className="p-6 border-t border-slate-100">
        <button
          ...
          disabled={loading}
          className="w-full bg-[#14141A] text-white font-bold py-4 rounded-2xl text-base hover:bg-[#2a2a2a] disabled:opacity-60 transition-all active:scale-95"
        >
